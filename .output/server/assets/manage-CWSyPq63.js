...
const findNextPendingSubStage = (currentSubStage, subStages, allSubStages, routes) => {
  ...
  const findFirstNonSkippable = () => allSubStages.find(({ subStage }) => !SKIPPABLE_SUB_STAGES.has(subStage)) ?? null;
  if (!currentSubStage) {
...
const getNextStageSubStage = (currentStage, routes) => {
  ...
  return nextStage?.subSteps[0] ? { stage: nextStage.value, sub_stage: nextStage.subSteps[0].name } : null;
};
...
export {
  formatDate as a,
  findNextPendingSubStage as b,
  getSubStageStatuses as c,
  formatDateWithTime as f,
  getNextStageSubStage as g,
  isSubStageAccessible as i,
  navigateTo as n,
  sliceFilename as s
};
