import { f as fromWebHandler } from '../_/nitro.mjs';
import { createMemoryHistory } from '@tanstack/history';
import { json, mergeHeaders } from '@tanstack/router-core/ssr/client';
import { createSerializationAdapter, joinPaths, trimPath, isRedirect, isResolvedRedirect, isNotFound, rootRouteId, executeRewriteInput, defaultSerovalPlugins, makeSerovalPlugin } from '@tanstack/router-core';
...
async function loadVirtualModule(id) {
  switch (id) {
    case VIRTUAL_MODULES.startManifest:
      return await import('../_/_tanstack-start-manifest_v-BfUU6foD.mjs');
...
function createStartHandler(cb) {
  ...
  const getEntries = async () => {
    if (routerEntry === null) {
      routerEntry = await import('../_/router-e7zdrxGz.mjs').then((n) => n.r);
    }
    if (startEntry === null) {
  ...
  const startRequestResolver = async (request, requestOpts) => {
    ...
    const getRouter = async () => {
      ...
      const origin = (_a2 = router.options.origin) != null ? _a2 : getOrigin();
      router.update({
        history,
        isShell,
        isPrerendering,
